...
public class ColorUtil {
    ...
    @ColorInt
    public static int changeColorAlphaTo25(@ColorInt int color1) {
         /*First byte in the color int is responsible for the transparency: 0 - completely transparent, 255 (0xFF) – opaque.
           In the first part ("&" operation) we set the first byte to 0 and left other bytes untouched.
           In the second part we set the first byte to 0x33 which is the 25% of 0xFF (255 * 0.25 ≈ 64).
         */
        return (color1 & 0x00FFFFFF) | 0x40000000;
    }

    @ColorInt
    public static int generatePrimaryTintedColorOverlay(@ColorInt int color) {
         /*First byte in the color int is responsible for the transparency: 0 - completely transparent, 255 (0xFF) – opaque.
           In the first part ("&" operation) we set the first byte to 0 and left other bytes untouched.
           In the second part we set the first byte to 0x0D which is the 5% of 0xFF (255 * 0.05 ≈ 13).
         */
        return (color & 0x00FFFFFF) | 0x0D000000;
    }
}
