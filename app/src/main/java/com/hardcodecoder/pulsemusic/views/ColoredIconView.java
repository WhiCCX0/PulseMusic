...
public class ColoredIconView extends AppCompatImageView {
    ...
    private void initialize(Context context, @Nullable AttributeSet attributeSet) {
        ...
        int paddingPixels = context.getResources().getDimensionPixelSize(R.dimen.colored_icon_padding);
        ...
        boolean desaturated = AppSettings.getAccentDesaturatedColor(context) && typedArray.getBoolean(R.styleable.ColoredIconView_desaturatedColorInDarkMode, true);

        if (desaturated) {
            iconColor = context.getResources().getColor(R.color.darkColorBackground);
            iconBackgroundColor = ColorUtil.mixColors(iconBackgroundColor, Color.WHITE, 0.45f);
        } else
